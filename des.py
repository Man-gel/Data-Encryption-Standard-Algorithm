...
def stringTobits(s): #s ->(string) mensaje + bits de relleno. Regresa list de strings, c/u = c/letra del msg en su representación binaria.
    res = []
    mask='00000000'
    for c in s:
    ...
    '''
def faltaRelleno(msg): #msg ->(string) mensaje del usuario. Regresa(entero) cant de caracteres que faltan para que len(msg) sea múltiplo de 8 
    lets = 0
    letsXblq = TAM_BLQ / 8
    for w in msg:
        for l in w:
            lets += 1
    if lets % letsXblq == 0: #msg -> (string). Regresa 0 si es múltiplo del tamaño de bloque y no necesita rellenarse, SINO regresa x (1<x<7)
        return 0
    else:
...
def bloques64( part ): #part -> list de strings de len()=8 (c/u = 1 caracter del msg como octeto). Regresa list de strings con len()=64
    bloque=''
    bits=0
    blq64 = []
...
def calcularClaves(k): #k -> string o list de chars len()=64, key original. Regresa list con 16 keys (strings len()=48 c/u).
    kList= []
    kPerm = []
    izq=2
    for b in range( len(PC1_TABLA) ):
        kPerm.append( k[ PC1_TABLA[b] ] )
    ...
    for r in range(N_RONDAS):        
        ...
        kConc.extend(D0)
        Ki=[]
        for i in range( len(PC2_TABLA) ):
            Ki.append( kConc[PC2_TABLA[i]] )
        kList.append(Ki)
    return kList

def pInicial(blq): #blq -> string de len()=64 con '0'|'1'. Regresa list de enteros con 0 | 1 de len()=64
    blqPerm = []
    for b in range( len(blq) ):
        blqPerm.append(int( blq[ IP_TABLA[b] ] ))        
    return blqPerm

def pFinal(blq): #blq -> string de len()=64 con '0'|'1'. Regresa list de enteros con 0 | 1 de len()=64
    blqPerm = []
    for b in range( len(blq) ):
        blqPerm.append(int( blq[ IP_INV_TABLA[b] ] ))        
    return blqPerm

def expansion(blq): #blq ->  list = 1 bloque de 32 bits(como enteros), Regresa list de 48 bits (como enteros)    
    blqExp=[]
    blqExp.append( blq[-1] )
    for b in range( len(blq) ):
        if b in E_TABLA:
            blqExp.append( blq[b] )
            blqExp.append( blq[ E_TABLA[b] ] )
            blqExp.append( blq[b] )
        else:
            blqExp.append( blq[b] )
    blqExp.append( blq[0] )
...
def xor(bits,k): #bits -> list de enteros(1|0). k -> list o string solo con chars '0'|'1'.Siempre con len(bits). Regresa list de enteros 0|1.
    _xor = []
...
def sustitucion(bits): #bits -> list de enteros 0 | 1 con len()=48. Regresa list con len()=32 con enteros 0| 1
    bitSust = []
    mask = '0000'
    cont = 0
    while cont < 48:
...
def permutacion(blq): #blq -> list de len()=32 con enteros 1 | 0. Regresa una estructura igual permutada.
    bPerm = []
    for b in range( len(blq) ):
        bPerm.append( blq[ PERM_TABLA[b] ] )
    return bPerm

def intercambiarBloques(blqL, blqR): #blqL, blqR -> list len()=32 c/u (enteros).Regresa string len()=64 con bloques intercambiados
    blqExch = ''
    for r in blqR:
        blqExch += str(r)
    for l in blqL:
...
def encriptar(m,k):    
    ...
    while ronda < N_RONDAS:        #i desde 1 a 16
        if ronda > 0:
...
def hexTobin(hx):#hx ->(string) msg|key en hexadecimal. Regresa list de strings, c/u = c/par letras de hx(1octeto) en representación binaria.
    toHex = []
    mask='00000000'
    x=0
    while x < len(hx):
...
def binToStr(b):#b ->  list de enteros len()=64. Regresa string equivalente a las letras de cada octeto de la lista.
    w=''
    oc=''
...
def keyHextoBin(k): #k -> string con la clave en hexadecimal. Regresa string len()=64 (representación binaria de la clave).
    kBin = ''
    mask = '00000000'
    i=0
    while i < len(k):
